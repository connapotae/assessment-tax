...
func (h *Handler) SetupDeductionHandler(c echo.Context) error {
	...
	case "k-receipt":
		m[deductType] = validates{condition: "required,gte=0,lte=100000", err: "amount must between 0 and 100,000"}
		res = DeductRes{KReceiptDeduction: a.Amount}
	default:
	...
	if err := validate.Var(a.Amount, m[deductType].condition); err != nil {
		return c.JSON(http.StatusBadRequest, Err{Message: m[deductType].err})
	}
